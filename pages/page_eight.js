...
export default class Page_8 extends Component {
    ...
    myQuestions = new QuestionList().questions[7];
    ...
    handleChange(index, answers) {
        ...
        this.props.handleChange(7, this.virtualState.answers, this.state.hidden);
        this.getCompletion();
    }
    ...
    getCompletion() {
        ...
        this.props.submitCompletion(7, validAnswer.length, hasAnswer.length);
    }
    ...
    switch_Widget = (widget, index) => {
        switch (widget.type)
        {
            ...
            case 'notification':
                return (
                    this.state.hidden[index].hidden === false ? (<Separator style={{alignItems: 'center'}}>
                        <Text>
                            {widget.text}
                        </Text>
                    </Separator>) : (<View/>)
